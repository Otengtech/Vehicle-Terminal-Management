  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">{vehicle.licensePlate}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{vehicle.year}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-500">VIN</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{vehicle.vin || 'N/A'}</p>
                </div>
              </div>
            </div>
          </CardBody>
        ...
        <Card>
          <CardBody>
            <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Location & Condition</h3>
            <div className="space-y-3">
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-900 dark:text-white">
                  Condition: {vehicle.condition} • Fuel: {vehicle.fuelLevel}
                </span>
              </div>
              <div className="flex items-center">
                <IoCalendarOutline className="h-4 w-4 text-gray-400 mr-2" />
                <span className="text-sm text-gray-900 dark:text-white">
                  Checked in: {vehicle.checkInTimestamp ? formatDate(vehicle.checkInTimestamp) : 'Not checked in'}
                </span>
              </div>
            </div>
          </CardBody>
        ...
        <Card>
          <CardBody>
            <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Assigned Driver</h3>
            {vehicle.assignedDriverId ? (
              <div className="space-y-2">
                ...
                <Button 
                  ...
                  size="sm" 
                  className="mt-2"
                  onClick={() => navigate(`/drivers/${vehicle.assignedDriverId._id}`)}
                >
                  View Driver
                </Button>
              </div>
      ...
      {vehicle.damages?.length > 0 && (
        <Card>
          <CardHeader>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Damage Reports</h2>
          </CardHeader>
          <CardBody>
            <div className="space-y-4">
              {vehicle.damages.map((damage, index) => (
                <div key={index} className="border-l-4 border-error-500 bg-error-50 dark:bg-error-900/20 p-4 rounded-r-lg">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-gray-900 dark:text-white">{damage.description}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Location: {damage.location} • Reported by: {damage.reportedBy?.fullName}
                      </p>
                    </div>
